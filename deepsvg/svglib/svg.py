...
from .svg_command import SVGCommandBezier, SVGCommandLine
...
class SVG:
    ...
    def animate(self, file_path=None, frame_duration=0.1, do_display=True):
        ...
        if do_display:
            ...
            ipd.display(ipython_display(src, fps=24, rd_kwargs=dict(progress_bar=True), autoplay=1, loop=1))
    ...
    def unit_triangle():
        ...
        triangle = SVGPath([
            SVGCommandLine(Point(.5, 0.), Point(1., 1.)),
            SVGCommandLine(Point(1., 1.), Point(0., 1.)),
            SVGCommandLine(Point(0., 1.), Point(.5 , 0.))
        ]).to_group()
        
        return SVG([triangle], viewbox=Bbox(1))
        
    @staticmethod
    def unit_inverted_triangle():
        
        triangle = SVGPath([
            SVGCommandLine(Point(.5, 1.), Point(0., 0.)),
            SVGCommandLine(Point(0., 0.), Point(1., 0.)),
            SVGCommandLine(Point(1., 0.), Point(.5 , 1.))
        ]).to_group()
